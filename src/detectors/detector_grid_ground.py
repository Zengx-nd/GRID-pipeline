...
class GRIDGroundDetector(Detector):
    def __init__(self):
        super().__init__(
            detector_name='GRID-Ground',
            short_name='ground_test',
            source_pattern='/labweb/labwork/labdata/GRIDGROUND/star/app/sample',
        ...
        self.wave_pattern = re.compile(b'\x2e\x2e\x33\xff.{552}\x22\xee\xff\x33', re.S)
    ...
    def find_and_classify_wav_frames(self, data_sci: bytes, valid_frame_data):
        
        event_frames = re.findall(self.wave_pattern, data_sci)
        ...
        for frame in event_frames:
            # 校验和检查
            check_sum = struct.unpack('>H', frame[554: 556])[0]
            if check_sum != crc16_xmodem_nd(np.frombuffer(frame[0: 554], dtype=np.uint8)):
                # checksum error
                checksum_error_count += 1
                continue
            ...
            pps_for_utc = struct.unpack('>I', frame[8:12])[0]
            if utc == 0:
                time_error_count += 1
                continue

            timestamp_offset = struct.unpack('>Q', frame[12:20])[0]

            valid_events.append(frame)
            event_utcs.append(pps_for_utc)
        ...
        valid_frame_data['EVENTS'] = [event_utcs, event_timestamps, valid_events, 560]
    ...
    def extract_packets(self, filename: str) -> dict[str, list]:
        ...
        print('len of data is ', len(data_lvds))
        # 输出调试信息
        print(f'detector file extraction logs:')
        
        with open('output.txt', 'a') as f:
            print(data_lvds[:500000], file=f)
        ...
        valid_frame_data = {}
        # 数据帧查找与处理
        self.find_and_classify_wav_frames(data_frame, valid_frame_data)
        # self.find_and_classify_tte_frames(data_frame, valid_frame_data)
        # self.find_and_classify_spec_frames(data_frame, valid_frame_data)
        # self.find_and_handle_hk_frames(data_frame, valid_frame_data)
    ...
    def unpack_packets(self, packet_type: str, raw_packets) -> dict[str, list]:
        ...
        if packet_type == 'EVENTS':
            unpacked = unpack_wave_packets(raw_packets)
...
def unpack_wave_packets(raw_packets) -> dict[str, list]:
    ...
    for pkt in raw_packets:
        ...
        pps_for_utc = struct.unpack('>I', pkt[8:12])[0]
        timestamp_for_utc = struct.unpack('>Q', pkt[12:20])[0]
        channel = struct.unpack('>H', pkt[20:22])[0]
        timestamp_trigger = struct.unpack('>I', pkt[540:544])[0]
        adc_max = struct.unpack('>H', pkt[544:546])[0]
        adc_base = struct.unpack('>H', pkt[546:548])[0]

    
        fields['UTC']               .append(utc)
        fields['PPS_UTC']           .append(pps_for_utc)
        fields['TIMESTAMP_REF']     .append(timestamp_for_utc)
        fields['TIMESTAMP_TRIG']    .append(timestamp_trigger)
        fields['CHANNEL']           .append(channel)
        fields['ADC_VALUE']         .append(adc_max)
    ...
    unpacked = {
        'UTC': ['J', 's', fields['UTC']],
        'PPS_UTC':['J', '', fields['PPS_UTC']],
        'TIMESTAMP_REF': ['K', '', fields['TIMESTAMP_REF']],
        'TIMESTAMP_TRIG': ['J', '', fields['TIMESTAMP_TRIG']],
